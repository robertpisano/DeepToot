# a neural net architecture describes the architecture (lstm vs dense)
# as well as how the model shape of the architecture changes (number of hidden layers, number of neurons per hidden layer)
# activation function
# loss function

# NB: as of right now we will have a one to one mapping with a strategy and as such an architecture will similarly need a factory which produces a concrete architecture based on the strategy
    # - however, we possibly forsee the need to switch out different architectures for the same strategy, which will call for an abstract factory, to produce a specific architecture based
